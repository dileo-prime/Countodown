...
public class JiraConnector extends AbstractConnector {
   ...
   protected void addNewJiraConnection(final String username, final String jiraAdress, final JiraConnection connection)
         throws IllegalArgumentException {
      final ImmutablePair<String, String> key = new ImmutablePair<String, String>(username, jiraAdress);
      final JiraConnection old = connections.put(key, connection);
      if (old != null) {
         connections.put(key, old);
         throw new IllegalArgumentException(
               "Connection for Username: " + username + ", and adress: " + jiraAdress + " already existing");
      }
   }
   ...
   protected int getTotalAmountOfResultsForAdress(final CharSequence adress, final JiraConnection connection) {
   ...
   protected Task issueToTask(final Issue i) {
      ...
      final Task task = new Task("" + i.id, JIRA, i.key, summary, assigne, dueDate, workToDo, fields.description);
