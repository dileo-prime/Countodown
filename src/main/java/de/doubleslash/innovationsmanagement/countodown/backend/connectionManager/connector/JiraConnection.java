...
public class JiraConnection {
   ...
   public JiraConnection(final String username, final String password, final String adress)
      ...
      if (username == null || username.isEmpty() || password == null || password.isEmpty()) {
         ...
         this.userPass = BASIC + Base64.encodeBase64((username + USERPASS_SEPERATOR + password).getBytes());
      }
   ...
   protected URL stringToURL(final String adress) throws MalformedURLException, UnsupportedEncodingException {
      return new URL(URLEncoder.encode(adress, "utf8").replace("%2F", "/").replace("%3A", ":").replace("%3F", "?")
   ...
   protected String fireURL(final URL url, final boolean useErrorStreamIfNecessarry) throws IOException {
      return fireURL(url, RequestMethod.GET, useErrorStreamIfNecessarry);
   }

   protected String fireURL(final URL url, final RequestMethod method, final boolean useErrorStreamIfNecessarry)
         throws IOException {
